...
export const deleteBlogImg = createAsyncThunk(
  ...
  async ({ blogId, publicId }, thunkAPI) => {
    try {
      return await blogService.deleteBlogImg(blogId, publicId);
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
    }
  }
...
export const blogSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteBlogImg.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(deleteBlogImg.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isError = false;
        state.isSuccess = true;
        state.deletedImg = action.payload;
      })
      .addCase(deleteBlogImg.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = true;
        state.isSuccess = false;
        state.message = action.error;
