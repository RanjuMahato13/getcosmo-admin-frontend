...
import { delImg, uploadImg, addImage } from "../features/upload/uploadSlice";
...
const Addblog = () => {
  ...
  //     blogImages.map((i) => {
  //       dispatch(addImage({ public_id: i.public_id, url: i.url }));
  //     });
  //   }
  // }, [blogImages]);
  ...
  useEffect(() => {
    if (blogImages) {
      const updatedBlogImages = blogImages.map((image) => ({
        ...image,
        existing: true,
      }));
